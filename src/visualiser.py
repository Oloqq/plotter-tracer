...
def visualize(moves, width, height, show=False):
    ...
    if grid:
        for x in range(width):
            draw.line([x*tile_size, 0, x*tile_size, outh],
                      fill=gridline_color, width=1)
        for y in range(height):
            draw.line([0, y*tile_size, outw, y*tile_size],
                      fill=gridline_color, width=1)
    ...
    for move in moves:
        ...
        if type(move) is Vec2D:  # it's a displacement
            ...
            draw.line(convert(pos, pad) + convert(move, pad),
                      fill=color, width=line_size)
            pos = move
        elif type(move) is str:  # it's a non-displacement operation
            match move:
                case 'pen up':
                    pen_down = False
                    draw.arc(circle_boundary(pos, dot_size),
                             180, 0, fill=(255, 0, 0), width=15)
                case 'pen down':
                    pen_down = True
                    draw.arc(circle_boundary(pos, dot_size), 0,
                             180, fill=(0, 255, 0), width=15)

    if show:
        img.show()
    now = datetime.now()
    img.save(f'data/moves_{now.strftime("%m-%d-%Y %H-%M-%S")}.png')


if __name__ == '__main__':
    from navigating import squiggler, closing_circles

    width = 117
    height = 117
    img = Image.open('data/out.png')
    # moves = squiggler(img)
    moves = closing_circles(img)
    # print(moves)
